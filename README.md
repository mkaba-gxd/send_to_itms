...
指定された Sample ID をキーにしてデータベースを検索し、検体情報を取得するため、データベースに登録がない検体に対しては実行できません。\
データベースの解析ステータスが終了(102)でない検体は除外されます。\
**このツールは正式な検証を経ていません。** 不具合等が生じた場合は適宜修正するか、DBから検体情報を検索してHDDにフォルダ構成を作成のうえ、[送付データ一覧](https://github.com/mkaba-gxd/send_to_itms#送付データ)を参照し、該当データを手動でrsync転送、チェックサムを作成してください。\
...
## エイリアスの作成（初回のみ）
**※ gxd_pipelineユーザーとguest_userユーザーは設定済なので実施しなくて良いです。**\
...
ANAL_STATUSが102でない、または CLINICAL_TRIAL_NAMEが「MONSTAR-SCREEN-3」でない検体は除外される。\
...
【-p オプションを付加しない場合】\
大きいデータ(fastq.gz,bam,vcf)のシンボリックリンクの作成のみ実行する。\
これらのデータはレポートを修正した場合でも変更されないので、レビューの前に重いデータだけ先に転送しておくためのオプションです。\
⇒ 作成されたデータセットを rsync -avLzu コマンドで解析データを送付用HDDへ転送する。\
⇒ レポートのアップロードが完了したら、-p オプションなしで再実行し、すべてのデータのコピーとチェックサムの作成を行う。
